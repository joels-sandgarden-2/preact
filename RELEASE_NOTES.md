...
* Fixed recovery during initial page load so delayed or failing content can continue cleanly even when no extra page nodes are available.
...
* Stabilized follow up updates so queued screen updates run in the right order before later renders.
